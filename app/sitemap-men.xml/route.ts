...
export async function GET() {
  try {
    ...
    // Get Men category and all its subcategories
    let menCategory: any[] = []
    let menId: string | null = null

    try {
      menCategory = await sql`
      ...
      menId = menCategory[0].id
    } catch (err) {
      console.error('Error fetching men category:', err)
      return new NextResponse('<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"></urlset>', {
        headers: { 'Content-Type': 'application/xml' },
      })
    }
    ...
    // Get all products in men's category and subcategories
    let products: any[] = []
    try {
      products = await sql`
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  <!-- Men's Collection: Premium men's clothing, formal wear, semi-formal apparel, unstitched and stitched collections -->
${urls.join('\n')}
</urlset>`
    ...
    console.error('Error generating men sitemap:', error)
    const baseUrl = process.env.NEXT_PUBLIC_SITE_URL || process.env.NEXT_PUBLIC_APP_URL || 'https://www.noirefit.com'
